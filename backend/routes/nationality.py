...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'LocalAgentCore', 'NationalityReclaimer', 'logic')))

from parse_nationality import parse_nationality
...
async def nationality_context(request: Request):
    ...
    affidavit = generate_affidavit(
        nationality=parsed["suggested_nationalities"][0] if parsed["suggested_nationalities"] else "American State National",
        full_name=full_name,
